...
- ユーザー関連
  ...
  - **店舗口コミ投稿機能**: ユーザーは飲食店に口コミを投稿できます。
  - **店舗口コミ削除機能**: ユーザーが飲食店に投稿した口コミを削除できます。
  - **Stripe決済機能**: ユーザーが飲食店の予約に対してオンラインで決済を行うことができます。
  - **飲食店一覧ソート機能**: ユーザーは飲食店をランダムや評価順で並べ替えて表示できます。
- 管理者関連
  - **店舗代表者登録**: 管理者は管理画面を通じて店舗代表者を登録できます。
  - **お知らせメール送信**: 管理者は管理画面からユーザーまたは店舗代表者にお知らせメールを送信できます。
  - **口コミ削除機能**: 管理者は管理画面からユーザーが投稿した口コミを削除できます。
  - **CSVインポート**: 管理者は管理画面からCSVインポートにより新しい店舗情報を追加できます。
- 店舗代表者関連
...
- **口コミ投稿**
  - 各店舗に対し、2回まで口コミを投稿することができます。投稿した口コミは編集、削除することができます。
...
- **CSVによる店舗新規登録方法**
  - 以下の形式でCSVファイルを作成し、店舗情報を一括登録できます。
    - **CSVファイルを作成**
      - ExcelやGoogleスプレッドシートを使用して、以下のフォーマットに従ったデータを作成します。完成したら、CSV形式で保存します。
      - **CSVファイルのフォーマット**
        - Excelやスプレッドシートの例
![art](csv-example.png)
        - CSVの例:以下は正しいCSVデータの例です。
          ```text
          店舗名,ユーザー名,地域,ジャンル,店舗概要,画像URL
          例：山田寿司,山田太郎,東京都,寿司,鮮度抜群のネタを提供する寿司店です。,https://example.com/image.jpg
          ```
      - CSVファイルには以下の列を含める必要があります。
        - **店舗名**:登録する店舗の名前（必須、最大50文字）。
        - **ユーザー名**:登録する店舗の代表者となる店舗代表者として登録済みのユーザーの名前（必須）。
        - **地域**:店舗の所在地。以下のいずれかを指定してください（必須）：
          - 東京都
          - 大阪府
          - 福岡県
        - **ジャンル**:「寿司」「焼肉」「イタリアン」「居酒屋」「ラーメン」のいずれか。
          ...
          - 居酒屋
          - ラーメン
        - **店舗概要**:店舗の説明文（必須、最大400文字）。
        - **画像URL**:店舗画像のURL（必須、jpegまたはpng形式）。
      - **CSVファイルの作成の注意点**
        - 必ずUTF-8形式で保存してください。
        - ヘッダー行（列名）は含めないでください。
        - 各列のデータを正確に入力し、フォーマットを守ってください。
      - **CSVファイルの登録方法**
        - 管理者アカウントでシステムにログインします。
        - 管理画面から「CSVインポート」を選択します。
        - 各列のデータを正確に入力し、フォーマットを守ってください。
        - エラーがない場合、登録が完了します。エラーが発生した場合、詳細なエラーメッセージが表示されます。
...
#### 1.
``` bash
git clone git@github.com:mdrgreen39/rese.git
```
...
#### 10. フロントエンドの依存関係をインストール
- src配下で以下を実行
``` bash
npm install
```

#### 11. 開発サーバーを起動(ローカル環境では起動したままにしてアプリを動かす)
- src配下で以下を実行以下を実行
``` bash
npm run dev
```
...
- 口コミ投稿のテスト用データについて
  - 口コミ投稿には、シーダーを使用して作成されるテストデータが含まれています。
  - 口コミ投稿の写真には、テスト目的のためにランダムな画像が使用されています。これらの画像は実際の投稿内容とは関係ありません。
  - テスト画像は `public/images/comments` ディレクトリに保存されています。
  - 本番環境では適切な画像を使用してください。
